...
import { CameraIcon, ChatIcon, GalleryIcon } from './Icons';
...
const BottomNavBar: React.FC = () => {
    ...
    return (
        <div className="absolute bottom-0 left-0 right-0 h-24 bg-black/80 backdrop-blur-md flex justify-around items-center border-t border-gray-800 z-20">
            <button onClick={() => setView('gallery')} className="flex flex-col items-center justify-center text-white transition-colors duration-200 w-20">
                <GalleryIcon isActive={currentView === 'gallery'} />
                <span className={cn('text-xs mt-1', currentView === 'gallery' ? 'text-yellow-400' : 'text-gray-400')}>Gallery</span>
            </button>
            ...
            <button id="onboarding-chat-nav" onClick={() => setView('chat')} className="flex flex-col items-center justify-center text-white transition-colors duration-200 w-20">
